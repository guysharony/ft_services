#!/bin/bash
destination=/tmp/gsharony

rm -rf $destination/.docker
rm -rf $destination/.minikube
rm -rf $destination/com.docker.docker
...
mkdir -p $destination/com.docker.docker
mkdir -p $destination/.docker

ln -sf $destination/com.docker.docker ~/Library/Containers/com.docker.docker
ln -sf $destination/.docker ~/.docker

pkill Docker
open -g -a Docker

export MINIKUBE_HOME=$destination
